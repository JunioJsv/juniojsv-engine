...
class Vector2fVbo(override val vao: Int, override val location: Int, override val divisor: Int) : Vbo() {
    init {
    ...
    fun update(data: List<Vector2f>) {
        val bytes = (data.size * Constants.VEC2_BYTE_SIZE).toLong()
        if (bytes == 0L) return
        GL.glBindBuffer(GL_ARRAY_BUFFER, id)
        ...
        try {
            ...
            for (vector2f in data) {
                buffer.putFloat(vector2f.x).putFloat(vector2f.y)
            }
        } catch (e: Exception) {
