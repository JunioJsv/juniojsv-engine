...
class TextureUnits {
    companion object {
         ...
         */
        val TYPES = arrayOf(GL_TEXTURE_2D, GL_TEXTURE_CUBE_MAP)
        ...
        fun getBindings(bindType: Int): List<Int> {
            val bindings = units.map { unit ->
                ...
                GL.glGetInteger(bindType)
            }
            GL.glActiveTexture(units[0])
            return bindings
        }
        ...
        fun unbindAll() {
            units.forEach { unit ->
                GL.glActiveTexture(unit)
                TYPES.forEach { type -> GL.glBindTexture(type, 0) }
            }
            GL.glActiveTexture(units[0])
        }
        ...
        fun bind(index: Int, texture: Texture): Boolean {
            ...
            GL.glActiveTexture(unit)
            val isAlreadyBound = texture.id == GL.glGetInteger(texture.getBindType())
